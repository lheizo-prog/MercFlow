...
interface Props { aberto: boolean; onNavegar: () => void; }
function Sidebar({ aberto, onNavegar }: Props) {
  ...
  useEffect(() => {
    ...
    const collapse = Collapse.getOrCreateInstance(sidebarRef.current, { toggle: false });
    if (aberto) collapse.show(); else collapse.hide();
  }, [aberto]);
  ...
  return (
    <aside id="menu-principal" ref={sidebarRef} className="collapse d-lg-block col-lg-2 bg-dark p-3">
      ...
      <nav className="nav nav-pills flex-column gap-1">
        {links.map((link) => (
          <NavLink key={link.to} to={link.to} end={link.to === "/"} className={({ isActive }) => `nav-link ${isActive ? "active" : "text-white"}`} onClick={onNavegar}>
            {link.label}
          </NavLink>
        ))}
      </nav>
