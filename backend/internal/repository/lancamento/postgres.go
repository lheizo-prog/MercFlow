...
func (r *LancamentoPostgresRepository) Criar(lancamento *models.Lancamento) (*models.Lancamento, error) {
	...
	err = tx.QueryRow(
		context.Background(),
		`INSERT INTO lancamentos (tipo, departamento_id, observacao, loja_id) VALUES ($1, $2, $3, $4) RETURNING id, data_lancamento`,
		lancamento.Tipo,
...
func (r *LancamentoPostgresRepository) BuscarID(id int) (*models.Lancamento, error) {
	...
	err := r.db.QueryRow(
		context.Background(),
		`SELECT id, loja_id, tipo, departamento_id, data_lancamento, observacao FROM lancamentos WHERE id = $1`,
		id,
	).Scan(
		&lancamento.ID,
		&lancamento.LojaID,
...
func (r *LancamentoPostgresRepository) Listar() ([]models.Lancamento, error) {
	rows, err := r.db.Query(
		context.Background(),
		`SELECT id, loja_id, tipo, departamento_id, data_lancamento, observacao FROM lancamentos ORDER BY data_lancamento DESC`,
	)
	if err != nil {
		return nil, err
	}
