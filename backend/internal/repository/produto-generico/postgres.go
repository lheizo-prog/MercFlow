...
func (r *PostgresProdutoGenericoRepository)Criar(p *models.ProdutoGenerico) (*models.ProdutoGenerico, error){
	err := r.db.QueryRow(context.Background(), "INSERT INTO produtos_genericos (nome, codigo) VALUES ($1, $2) RETURNING id", p.Nome, p.Codigo_Geral).Scan(&p.ID)
...
func (r *PostgresProdutoGenericoRepository)Listar() ([]*models.ProdutoGenerico, error){
	rows, err := r.db.Query(context.Background(), "SELECT id, nome, codigo FROM produtos_genericos ORDER BY nome;")
	if err != nil{
		return nil, err
	}
	defer rows.Close()
...
func (r *PostgresProdutoGenericoRepository)BuscarID(id int) (*models.ProdutoGenerico, error){
	...
	row := r.db.QueryRow(context.Background(), "SELECT id, nome, codigo FROM produtos_genericos WHERE id = $1;",id)
...
func (r *PostgresProdutoGenericoRepository)BuscarCodigo(codigo string) (*models.ProdutoGenerico, error){
	produto := &models.ProdutoGenerico{}
	row := r.db.QueryRow(context.Background(), "SELECT id, nome, codigo FROM produtos_genericos WHERE codigo = $1",codigo)
